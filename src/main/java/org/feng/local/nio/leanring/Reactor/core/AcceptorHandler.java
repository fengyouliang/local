...
public class AcceptorHandler implements Runnable {
    ...
    AcceptorHandler(SocketChannel socketChannel, Selector selector) throws IOException {
    ...
    public void run() {
        try {
            switch (status) {
                ...
                IOException e) { // 这里的异常处理是做了汇总，常出的异常就是server端还有未读/写完的客户端消息，客户端就主动断开连接，这种情况下是不会触发返回-1的，这样下面read和write方法里的cancel和close就都无法触发，这样会导致死循环异常（read/write处理失败，事件又未被cancel，因此会不断的被select到，不断的报异常）
    ...
    private void read() throws IOException {
        if (selectionKey.isValid()) {
            ...
            if (count > 0) {
                System.out.printf("收到来自 %s 的消息: %s%n", socketChannel.getRemoteAddress(), new String(readBuffer.array()));
                status = SEND;
    ...
    void send() throws IOException {
        if (selectionKey.isValid()) {
            sendBuffer.clear();
            sendBuffer.put(String.format("我收到来自%s的信息辣：%s,  200ok;", socketChannel.getRemoteAddress(), new String(readBuffer.array())).getBytes());
            sendBuffer.flip();

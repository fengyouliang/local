package org.feng.local.nio.leanring;
 ...
 * @Date: 02/02/2023 11:25
 * @Description:
 */
public class SimpleNioDemo {
        ...
        while (selector.select() > 0) { // 若收到就绪事件select返回“感兴趣”事件集合，否则阻塞当前线程
            ...
            while (iterator.hasNext()) {
                ...
                if (key.isAcceptable()) { // 当前就绪事件为连接事件
                    ...
                    System.out.printf("收到来自 %s 的消息: %s%n", socketChannel.getRemoteAddress(), new String(buffer.array()));
                }
                keys.remove(key);
            }
        }
